...
ItsInterface::ItsInterface() : node_handle_(), private_node_handle_("~") {
  ...
  if (create_subscriber_) sub_ = private_node_handle_.subscribe("message", 1, &ItsInterface::messageCallback, this);
...
void ItsInterface::timerCallback(const ros::TimerEvent& event) {

  ROS_INFO("Publishing message at time %f", event.current_real.toSec());

  std_msgs::String msg;
  msg.data = "CarlaItsInterface is running";

  pub_.publish(msg);
}


void ItsInterface::messageCallback(const std_msgs::String& msg) {
